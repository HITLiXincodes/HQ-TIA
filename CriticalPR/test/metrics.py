...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("ref_dir", type=str, nargs="?", help="Path to reference image folder")
    parser.add_argument("gen_dir", type=str, nargs="?", help="Path to generated image folder")
    parser.add_argument(
        "--device",
        type=str,
        default=None,
    ...
    parser.add_argument("--batch_size", type=int, default=None, help="Batch size for per-image metrics")
...
def main() -> None:
    ...
    ref_dir = args.ref_dir or metrics_config["ref_dir"]
    gen_dir = args.gen_dir or metrics_config["gen_dir"]
    device = args.device or metrics_config["device"]
    batch_size = args.batch_size if args.batch_size is not None else metrics_config["batch_size"]
    num_workers = args.num_workers if args.num_workers is not None else metrics_config["num_workers"]
    output_json = args.output_json if args.output_json is not None else metrics_config["output_json"]
    output_txt = args.output_txt if args.output_txt is not None else metrics_config["output_txt"]
